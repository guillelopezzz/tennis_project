...
def puntos_ranking(dataset, dataset_r, year):
    ...
    for i in range(X_df.shape[0]):
        ...
        winner_id, loser_id = X_df.iloc[i]['winner_id'], X_df.iloc[i]['loser_id']

        # Obtener el año y el mes del partido
        year_month = dataset.iloc[i]['year_month']
        ...
        if not winner_points.empty:
            X_df.iloc[i, X_df.columns.get_loc('winner_points')] = winner_points.values[0]
        if not loser_points.empty:
            X_df.iloc[i, X_df.columns.get_loc('loser_points')] = loser_points.values[0]
